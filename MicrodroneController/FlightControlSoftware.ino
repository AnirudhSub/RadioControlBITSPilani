...
float thr_des=0 , roll_des = 0, yaw_des = 0, pitch_des = 0;
double gyro_x_err, gyro_y_err, gyro_z_err;
...
double pitch_angle,roll_angle,yaw_angle, pitch_angle_deg, roll_angle_deg, yaw_angle_deg;
...
float Ki_roll_angle = 0.05;    //Roll I-gain - angle mode
float Kd_roll_angle = 0.3;   //Roll D-gain - angle mode

float Kp_pitch_angle = 0.4;   //Pitch P-gain - angle mode
float Ki_pitch_angle = 0.05;   //Pitch I-gain - angle mode
float Kd_pitch_angle = 0.3;  //Pitch D-gain - angle mode
...
const int m_fl_pin = 33, m_bl_pin = 32, m_fr_pin = 27, m_br_pin = 13; 
...
void loop() {
  ...
  gyro_x = (g.gyro.x - gyro_x_err);

  gyro_y = (g.gyro.y - gyro_y_err);

  gyro_z = (g.gyro.z - gyro_z_err);
  ...
  pitch_angle_deg = pitch_angle * 180.0 / 3.14;
  roll_angle_deg = roll_angle * 180.0 / 3.14;
  yaw_angle_deg = yaw_angle * 180.0 / 3.14;
...
void controlANGLE() {
  ...
  error_roll = roll_des - roll_angle_deg;
  integral_roll = integral_roll_prev + error_roll*dt;
  if (thr_des < 0.2) {   //Don't let integrator build if throttle is too low
    integral_roll = 0;
  }
  integral_roll = constrain(integral_roll, -i_limit, i_limit); //Saturate integrator to prevent unsafe buildup
  derivative_roll = gyro_x * 180.0 / 3.14;
  roll_PID = 0.05*(Kp_roll_angle*error_roll + Ki_roll_angle*integral_roll - Kd_roll_angle*derivative_roll); //Scaled by .01 to bring within -1 to 1 range

  //Pitch
  error_pitch = pitch_des - pitch_angle_deg;
  integral_pitch = integral_pitch_prev + error_pitch*dt;
  if (thr_des < 0.2) {   //Don't let integrator build if throttle is too low
    integral_pitch = 0;
  }
  integral_pitch = constrain(integral_pitch, -i_limit, i_limit); //Saturate integrator to prevent unsafe buildup
  derivative_pitch = gyro_y * 180.0 / 3.14;
  pitch_PID = 0.05*(Kp_pitch_angle*error_pitch + Ki_pitch_angle*integral_pitch - Kd_pitch_angle*derivative_pitch); //Scaled by .01 to bring within -1 to 1 range

  //Yaw, stablize on rate from GyroZ
  error_yaw = yaw_des - (gyro_z * 180.0 / 3.14);
  ...
  yaw_PID = 0.05*(Kp_yaw*error_yaw + Ki_yaw*integral_yaw + Kd_yaw*derivative_yaw); //Scaled by .01 to bring within -1 to 1 range
...
void constrain_des_states(){
  ...
  pitch_des = constrain(thr_des,-1,1);
  roll_des = constrain(thr_des,-1,1);
  yaw_des = constrain(thr_des,-1,1);
}